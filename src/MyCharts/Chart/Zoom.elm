module MyCharts.Chart.Zoom exposing (..)
...
-- chartAttrs : Model -> (Msg -> msg) -> List (CA.Attribute container data msg)
-- A funcao gera um tipo de variavel todo doido ai deixei sem definicao
chartAttrs model msgToMainMsg =
    [ CA.htmlAttrs [ HA.style "cursor" "crosshair" ]
    , CE.onMouseDown (msgToMainMsg << OnDown) CE.getCoords
    , CE.onMouseMove (msgToMainMsg << OnMove) CE.getCoords
    , CE.onMouseUp (msgToMainMsg << OnUp) CE.getCoords
...
-- chartElements : Model -> (Msg -> msg) -> List (CA.Attribute container data msg)
-- A funcao gera um tipo de variavel todo doido ai deixei sem definicao
chartElements model msgToMainMsg = 
    ...
    , case getCurrentWindow model of
        Just ( start, end ) ->
          C.htmlAt .max .max -10 -10
            ...
            [ H.button
                [ HE.onClick <| msgToMainMsg OnReset
